...
//    private TextView gugeGaochengText;
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                if(oneBodyFireList!=null && oneBodyFireList.size()!=0){
                    OneBodyFire oneBodyFire = oneBodyFireList.get(0);
                    if(oneBodyFire.getAlarmLatitude()!=null&&oneBodyFire.getAlarmLatitude()!=0 && oneBodyFire.getAlarmLongitude()!=null&&oneBodyFire.getAlarmLongitude()!=0){
                        flyToLngLat(oneBodyFire.getAlarmLongitude(),oneBodyFire.getAlarmLatitude());
                    }else{
                        flyToLngLat(CommonData.lng,CommonData.lat);
                    }
                }else if(weixingModelList!=null&&weixingModelList.size()!=0){
                    WeixingModel weixingModel = weixingModelList.get(0);
                    if(weixingModel.getLatitude().equals("null")||weixingModel.getLatitude().equals("0")||weixingModel.getLongitude().equals("null")||weixingModel.getLongitude().equals("0")){
                        flyToLngLat(CommonData.lng,CommonData.lat);
                    }else{
                        try{
                            flyToLngLat(Double.parseDouble(weixingModel.getLongitude()),Double.parseDouble(weixingModel.getLatitude()));
                        }catch (Exception e){
                            flyToLngLat(CommonData.lng,CommonData.lat);
                        }
                    }
    ...
    private void getBroads() {
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL+"/auth/api/auth/user/get/userAreaNew");
        ...
        x.http().get(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                Log.e(TAG, "onSuccess: setBoards = " + result );
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        Log.e(TAG, "run: setBoards？" );
                        dWebView.callHandler("setBoards", new Object[]{array}, new OnReturnValue<String>() {
                            @Override
                            public void onValue(String retValue) {
                                Log.e(TAG, "onValue:  setBoards" + retValue);
                            }
                        });
                    }
                },10000);
...
//                getContext().sendBroadcast(intent);
    ...
    private void flyToLngLat(double lng ,double lat){
        final JSONObject jsonObject = new JSONObject();
        JSONObject posObj = new JSONObject();
        try {
            posObj.put("lat", lat);
    ...
    class myReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                ...
                double[] doubles = new LatLngChangeNew().calBD09toWGS84(position.getDouble("lat"), position.getDouble("lng"));
                position.put("lat",doubles[0]);
                position.put("lng",doubles[1]);
                jsonObject.put("position",position);

                //Toast.makeText(context, "广播已经接收", Toast.LENGTH_SHORT).show();
                Log.i("onReceive: ", msg);
                dWebView.callHandler("GPSpoint", new Object[]{new Gson().toJson(jsonObject.toString())}, new OnReturnValue<String>() {
                    @Override
                    public void onValue(String retValue) {
                        Log.d("jsbridge", "call succeed,return value is " + retValue);
                    }
