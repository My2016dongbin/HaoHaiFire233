...
//测距
dsBridge.register('ceju', function (data) {
    measureDisNew();
});
//测面
dsBridge.register('cemian', function (data) {
    measureAreaNew();
});
//清除
dsBridge.register('cedelete', function (data) {
    measureAreaNew();
});
 //地图打点
...
//测距
function measureDisNew(clampMode = 0 /* 空间模式*/) {
    // clampMode = 1 /* 贴地模式*/
    if (window.handlerDis) {
        window.handlerDis.clampMode = clampMode
        window.handlerDis.activate()
        return
    }
    this.createDisHandler(clampMode)
}
function createDisHandler(clampMode){
    const handlerDis = new Cesium.MeasureHandler(viewer, Cesium.MeasureMode.Distance, 1)
    handlerDis.clampMode = clampMode
    ...
    handlerDis.activeEvt.addEventListener((isActive) => {
        this.setCursorByActive(isActive)
    })
    window.handlerDis = handlerDis
    window.handlerDis.activate()
}
function setCursorByActive(isActive) {
    if (!viewer) return
    // if (isActive === true) {
    ...
    if (isActive === true) {
        ...
        $('.cesium-viewer').removeClass('measureCur').addClass('measureCur')
    } else {
        viewer.enableCursorStyle = true
        // $('body').removeClass('measureCur')
        $('.cesium-viewer').removeClass('measureCur')
    }
}
function measureAreaNew(clampMode = 0) {
    if (window.handlerArea) {
        window.handlerArea.clampMode = clampMode
        window.handlerArea.activate()
        return
    }
    this.createAreaHandler(clampMode)
}
function createAreaHandler(clampMode) {
    // 初始化测量面积
    const handlerArea = new Cesium.MeasureHandler(viewer, Cesium.MeasureMode.Area, clampMode)
    handlerArea.clampMode = clampMode
    window.handlerArea = handlerArea
...
function measureHandlerClear() {
     window.handlerDis();
     window.handlerDis.clear();
    window.handlerDis.deactivate();
    window.handlerArea && window.handlerArea.clear() && window.handlerArea.deactivate()
}
