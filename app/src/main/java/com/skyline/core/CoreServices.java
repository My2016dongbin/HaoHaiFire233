...
public class CoreServices implements ComponentCallbacks2, LocationListener {
    ...
    // Public functions, should be called from C++ (see sl_jnisrv.h file) 公共函数应该从C++调用（请参阅sl_jnisrv.h文件）
    // ------------------------------------------------------------------
    ...
    public static void Init(Activity o) {
        ...
        if (coreServices == null) {
            coreServices = new CoreServices();    //core 核心
            appContext.registerComponentCallbacks(coreServices);
        }

        if (jexl == null) {
            jexl = new JexlEngine();    //表达式计算
            jexl.setSilent(true);    //设置在评估过程中引发错误时引擎是否引发JexlException。
            jexl.setLenient(true);    //设置此引擎是否将未知变量，方法和构造函数视为错误或将其评估为空值或零。
        }

		/* StrictMode这个类可以用来帮助开发者改进他们编写的应用，并且提供了各种的策略,
         这些策略能随时检查和报告开发者开发应用中存在的问题，
		 比如可以监视那些本不应该在主线程中完成的工作或者其他的一些不规范和不好的代码。*/

		 /* 为了能在应用中启用和配置StrictMode,开发者最好尽可能在应用程序的生命周期的早段使用,
		 方法是调用StrictMode的方法setThreadPolicy。
		 当使用常用监控类的时候，一个最好的调用时机，是在应用中入口和activities被调用前进行。
		 比如在一个应用程序中，可以把代码放在启动Activity类的onCreate()方法中 */
    ...
    public static long java_sharedSetValue(String key, byte[] val, int type, int val_size) {
        ...
        SharedPreferences settings = coreActivity.getSharedPreferences(appName, Context.MODE_PRIVATE);
        ...
        switch (type) {
            ...
            case 0x00000020: // REG_EXPAND_SZ
                try {
                    editor.putString(key + "_value", new String(val, "UTF-8"));
