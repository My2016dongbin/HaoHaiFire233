...
public final class ILinearRing extends TEIUnknownHandle {
    private ILinearRing(int handle)
    {
        super(handle);
    }    
    public static ILinearRing fromHandle(int handle)
    {
        if(handle == 0)
            return null;
        return new ILinearRing(handle);
    }
    private final static java.util.UUID IID = java.util.UUID.fromString("84CE9DF9-65AD-11D5-85C1-0001023952C1");
    ...
    public ISpatialRelation getSpatialRelation() throws ApiException    //得到空间关系
    {
        checkDisposed();
