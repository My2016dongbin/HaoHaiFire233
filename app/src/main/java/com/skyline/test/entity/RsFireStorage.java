...
import com.skyline.teapi.MsgTargetPosition;
...
public class RsFireStorage {
    public static final RsFireStorage defaultStorage=(new RsFireStorage()).new RsFireStorageFeatureLayer();
    public int[] iconList() {
        return new int[]{R.drawable.icon_rsfire};
    }

    protected class RsFireStorageFeatureLayer extends RsFireStorage implements ISGWorld.OnLoadFinishedListener, com.skyline.test.entity.RsFireStorageFeatureLayer {
        private IFeatureLayer favorites;

        public RsFireStorageFeatureLayer() {
            ...
            UI.runOnRenderThread(new Runnable() {
                ...
                public void run() {
                    ...
                    ISGWorld.getInstance().addOnLoadFinishedListener(RsFireStorageFeatureLayer.this);
                }
            });
        }

        private String getRsFiresPath() {
            String baseDir = Environment.getExternalStorageDirectory() + File.separator + "/Android/data/com.skyline.terraexplorer/files";
            String favoritesPath = baseDir + File.separator + "RsFires.shp";
        ...
        public void OnLoadFinished(boolean bSuccess) {
            ...
            if (new File(getRsFiresPath()).exists() == false) {
            ...
            loadFavoritesLayer("TEPlugName=OGR;LayerName=RsFires;FileName=" + getRsFiresPath(), imagesPath);
        }

        private void loadFavoritesLayer(String connectionString, String imagesPath) {
            try {
                favorites = ISGWorld.getInstance().getCreator().CreateFeatureLayer("RsFires", connectionString, ISGWorld.getInstance().getProjectTree().getHiddenGroupID());
                ...
                if (new File(getRsFiresPath()).delete())
                    OnLoadFinished(true);
            }
        }

        private void createFavoritesLayerSHP() {
            favorites = ISGWorld.getInstance().getCreator().CreateNewFeatureLayer("RsFires", LayerGeometryType.LGT_POINT, "TEPlugName=OGR;FileName=RsFires.shp", ISGWorld.getInstance().getProjectTree().getHiddenGroupID());
            ...
            String sourcePath = ISGWorld.getInstance().getApplication().getDataPath() + File.separator + "FeatureLayers/RsFires.shp";
            FileUtils.CopyFiles(new File(sourcePath).getParent(), "RsFires", new File(getRsFiresPath()).getParent());
        }

        public RsFireItem[] getAll() {
            return UI.runOnRenderThread(new Callable<RsFireItem[]>() {
                @Override
                public RsFireItem[] call() throws Exception {
                    IFeatures features = favorites.getFeatureGroups().getPoint().GetCurrentFeatures();
                    int count = features.getCount();
                    RsFireItem[] result = new RsFireItem[count];
        ...
        private RsFireItem favoriteItemFromFeature(IFeature feature) {
            if (feature == null)
                return null;

            try {
                RsFireItem item = new RsFireItem();
        ...
        private void updateOrCreateFeatureFromFavoriteItem(RsFireItem item) {
            if (item == null)
                return;
        ...
        public void saveItem(final RsFireItem item) {
            UI.runOnRenderThread(new Runnable() {
                @Override
                public void run() {
                    updateOrCreateFeatureFromFavoriteItem(item);
                    favorites.Save();
                    Intent intent = new Intent(RsFireItem.FavoriteChanged.getAction(0));
                    intent.putExtra(RsFireItem.FAVORITE_ID, item.id);
        ...
        public RsFireItem getItem(final String itemId) {
            if (itemId == null)
                return null;
            return UI.runOnRenderThread(new Callable<RsFireItem>() {
                @Override
    ...
    public String descriptionForIcon(int icon) {
        ...
        switch (icon) {
            ...
            case 5:
                return TEApp.getAppContext().getString(R.string.favorite_icon_star_fire);
        }
        return "";
    }

    public void saveItem(RsFireItem item) {
    ...
    public RsFireItem getItem(String itemId) {
        return null;
    }
}
