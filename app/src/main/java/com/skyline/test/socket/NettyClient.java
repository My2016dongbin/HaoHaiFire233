...
public class NettyClient {
	...
	final String ip= SPUtils.getInstance(SettingsTool.PREFERENCES_NAME).getString(Constance.USER_INITIAL_IP);
	final int port=SPUtils.getInstance(SettingsTool.PREFERENCES_NAME).getInt(Constance.USER_INITIAL_PORT);
	...
	public  boolean sendMsgToServer(PlottingItem.PlottingBean.Builder bean, ChannelFutureListener listener)
	{
		boolean flag = channel != null && isConnect;
		if (flag) {
			//ByteBuf buf = Unpooled.copiedBuffer(data);
			channel.writeAndFlush(bean).addListener(listener);
		}
		return flag;
	}
	public boolean sendMsgToServer(Object msg, ChannelFutureListener listener)
	{
		boolean flag = channel != null && isConnect;
		if (flag) {
			//ByteBuf buf = Unpooled.copiedBuffer(data);
