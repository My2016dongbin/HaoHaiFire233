...
import com.bean.GpsLocationBuf;
...
public class LocationTracker {
	...
	private LocationListener locationListener = new LocationListener() {
		...
		public void onLocationChanged(Location location) {
			...
			GpsLocationBuf.GpsBean.GpsContent.Builder gpsbean= GpsLocationBuf.GpsBean.GpsContent.newBuilder();
			String phoneNumber= SPUtils.getInstance(SettingsTool.PREFERENCES_NAME).getString(Constance.USER_PhoneNumber);
			gpsbean.setPhoneNumber(phoneNumber);
			GpsLocationBuf.GpsBean.PointLonLat.Builder pointLonLat=GpsLocationBuf.GpsBean.PointLonLat.newBuilder();
			pointLonLat.setLon(location.getLongitude());
			pointLonLat.setLat(location.getLatitude());
			pointLonLat.setAlt(location.getAltitude());
			gpsbean.setPoints(pointLonLat.build());
			NettyClient.getInstance().sendMsgToServer(Protocol.generateSend(3, gpsbean.build().toByteString()), new ChannelFutureListener() {
				@Override
