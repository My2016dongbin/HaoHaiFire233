...
public class AddbsfjActivity extends BaseActivity implements DatePicker.OnDateChangedListener{
    private ActionBar actionBar;
    private ActivityBsfjAddBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_bsfj_add);
                ...
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        if (uriChooseList.size() == 2){
                            Toast.makeText(AddbsfjActivity.this, "最多可以添加两张图片", Toast.LENGTH_SHORT).show();
                        }else {
                            addImage();
                        }
                    }
    ...
    private void postDataToService() {
        if (binding.mcEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写名称", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        if (binding.jianchashijianView.getText().toString().equals("")){
            Toast.makeText(this, "请填写分节时间", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("sectionName",binding.mcEdit.getText().toString());
            jsonObject.put("sectionLength",binding.chdEdit.getText().toString());
            jsonObject.put("sectionTime",binding.jianchashijianView.getText().toString()+" 00:00:00");
            jsonObject.put("sectionPerson",binding.fjrEdit.getText().toString());
            jsonObject.put("treeLocation",binding.smwzEdit.getText().toString());
            jsonObject.put("sectionLocation",binding.fjddEdit.getText().toString());
        ...
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "/forestry/api/sickTreeSectionInfo");
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        Toast.makeText(AddbsfjActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }else {
                        Toast.makeText(AddbsfjActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
...
//        Log.e(TAG, "onActivityResult:data ",data);
            ...
            Bitmap photo = null;
            try {
                photo = ImageUtils.getBitmapFormUri(AddbsfjActivity.this, uri);
            } catch (IOException e) {
            }
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        ...
                        Toast.makeText(AddbsfjActivity.this, "提交失败", Toast.LENGTH_SHORT).show();
                        progressDialog.dismiss();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
