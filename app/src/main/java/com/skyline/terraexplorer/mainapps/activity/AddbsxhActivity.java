...
import com.skyline.terraexplorer.databinding.ActivityBsxhAddBinding;
...
public class AddbsxhActivity extends BaseActivity implements DatePicker.OnDateChangedListener{
    private ActionBar actionBar;
    private ActivityBsxhAddBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_bsxh_add);
                ...
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        if (uriChooseList.size() == 2){
                            Toast.makeText(AddbsxhActivity.this, "最多可以添加两张图片", Toast.LENGTH_SHORT).show();
                        }else {
                            addImage();
                        }
                    }
    ...
    private void postDataToService() {
        if (binding.xhrEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写销毁人", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        if (binding.yskssjEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写销毁时间", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("person",binding.xhrEdit.getText().toString());
            jsonObject.put("type",binding.xhfsEdit.getText().toString());
            jsonObject.put("time",binding.yskssjEdit.getText().toString()+" 00:00:00");
            jsonObject.put("place",binding.xhddEdit.getText().toString());
        ...
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "/forestry/api/sickTreeDestructionInfo");
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        Toast.makeText(AddbsxhActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }else {
                        Toast.makeText(AddbsxhActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
...
//        Log.e(TAG, "onActivityResult:data ",data);
            ...
            Bitmap photo = null;
            try {
                photo = ImageUtils.getBitmapFormUri(AddbsxhActivity.this, uri);
            } catch (IOException e) {
            }
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        ...
                        Toast.makeText(AddbsxhActivity.this, "提交失败", Toast.LENGTH_SHORT).show();
                        progressDialog.dismiss();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
