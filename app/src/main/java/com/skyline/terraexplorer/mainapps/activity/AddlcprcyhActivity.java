...
import com.skyline.terraexplorer.databinding.ActivityAddlcprcyhBinding;
import com.skyline.terraexplorer.databinding.ActivityAddlcpzljgBinding;
...
public class AddlcprcyhActivity extends BaseActivity implements DatePicker.OnDateChangedListener{
    private ActionBar actionBar;
    private ActivityAddlcprcyhBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_addlcprcyh);
        initView();
        initDateTime();
        getDataFromService();
    }
    ...
    private void initView() {
                ...
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        if (uriChooseList.size() == 2){
                            Toast.makeText(AddlcprcyhActivity.this, "最多可以添加两张图片", Toast.LENGTH_SHORT).show();
                        }else {
                            addImage();
                        }
                    }
        ...
        RxViewAction.clickNoDouble(binding.bjcdwEdit)
                .subscribe(new Action1<Void>() {
                    @Override
                    public void call(Void aVoid) {
    ...
    private void postDataToService() {
        ...
        try {
            jsonObject.put("describe",binding.yhmsEdit.getText().toString());
            jsonObject.put("materialUsage",binding.syzkEdit.getText().toString());
            jsonObject.put("person",binding.yhrEdit.getText().toString());
            if (!binding.yhmjEdit.getText().toString().equals("")){
                jsonObject.put("area",Double.parseDouble(binding.yhmjEdit.getText().toString()));
            }
            jsonObject.put("treeSpecies",binding.yhszEdit.getText().toString());
            jsonObject.put("type",binding.yhlxEdit.getText().toString());
            jsonObject.put("date",binding.jianchashijianView.getText().toString()+" 00:00:00");
            jsonObject.put("remark",binding.kgqmsEdit.getText().toString());
            jsonObject.put("productionBaseName",binding.bjcdwEdit.getText().toString());
            jsonObject.put("productionBaseId",xmid);
            if (imglist.size()>0){
        ...
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "/forestry/api/forestProductDailyMaintenance" );
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        Toast.makeText(AddlcprcyhActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }else {
                        Toast.makeText(AddlcprcyhActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
...
//        Log.e(TAG, "onActivityResult:data ",data);
            ...
            Bitmap photo = null;
            try {
                photo = ImageUtils.getBitmapFormUri(AddlcprcyhActivity.this, uri);
            } catch (IOException e) {
            }
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        ...
                        Toast.makeText(AddlcprcyhActivity.this, "提交失败", Toast.LENGTH_SHORT).show();
                        progressDialog.dismiss();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (jsonObject1.getString("code").equals("200")) {
                        ...
                        Toast.makeText(AddlcprcyhActivity.this, "数据获取失败", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    private void showxxmcChangeDailog() {
        ...
        builder = new AlertDialog.Builder(this).setTitle("基地名称")
                .setSingleChoiceItems(items, choose2, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        Log.e(TAG, "onClick: "+i );
                        binding.bjcdwEdit.setText(xmjdTypeList.get(choose2).getName());
                        xmid = xmjdTypeList.get(choose2).getId();
                    }
                });
        builder.create().show();
    }
