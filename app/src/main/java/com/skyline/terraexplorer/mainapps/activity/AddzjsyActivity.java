...
import com.skyline.terraexplorer.databinding.ActivityAddzjsyBinding;
...
public class AddzjsyActivity extends BaseActivity implements DatePicker.OnDateChangedListener{
    private ActionBar actionBar;
    private ActivityAddzjsyBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_addzjsy);
        initView();
        getDataFromService();
        initDateTime();
    }
    ...
    private void postDataToService() {
        if (binding.zjzeEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写资金总额", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        if (binding.ysyjeEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写已使用资金额", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        if (binding.zfjeEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写支付金额", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        ...
        try {
            ...
            jsonObject.put("source",binding.zjlyEdit.getText().toString());
            jsonObject.put("amount",Double.parseDouble(binding.zjzeEdit.getText().toString()));
            jsonObject.put("used",Double.parseDouble(binding.ysyjeEdit.getText().toString()));
            jsonObject.put("isInPlace",yqrw);
            jsonObject.put("paymentItem",binding.zfxEdit.getText().toString());
            jsonObject.put("paymentAmount",Double.parseDouble(binding.zfjeEdit.getText().toString()));
            jsonObject.put("contractPerformance",binding.htzxqkEdit.getText().toString());
            jsonObject.put("basicAccountingWork",binding.kjgzEdit.getText().toString());
            jsonObject.put("internalControlSystem",binding.nkzdEdit.getText().toString());
            jsonObject.put("financialAccountingSystem",binding.nkzdEdit.getText().toString());
            jsonObject.put("remark",binding.kgqmsEdit.getText().toString());
        } catch (JSONException e) {
            e.printStackTrace();
        }
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "/forestry/api/projectFundUseManagement" );
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        Toast.makeText(AddzjsyActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }else {
                        Toast.makeText(AddzjsyActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
