...
import com.skyline.terraexplorer.databinding.ActivityBsfjAddBinding;
import com.skyline.terraexplorer.databinding.ActivityBsysAddBinding;
...
public class AddbsysActivity extends BaseActivity implements DatePicker.OnDateChangedListener{
    private ActionBar actionBar;
    private ActivityBsysAddBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_bsys_add);
    ...
    private void initView() {
                ...
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        if (uriChooseList.size() == 2){
                            Toast.makeText(AddbsysActivity.this, "最多可以添加两张图片", Toast.LENGTH_SHORT).show();
                        }else {
                            addImage();
                        }
                    }
        ...
        RxViewAction.clickNoDouble(binding.yskssjEdit)
                .subscribe(new Action1<Void>() {
                    @Override
                    public void call(Void aVoid) {
                        showDataDialog(binding.yskssjEdit);
                    }
                });
    ...
    private void postDataToService() {
        if (binding.yskssjEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写开始运输时间", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        if (binding.ysrEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写运输人", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("carrier",binding.ysrEdit.getText().toString());
            jsonObject.put("licencePlate",binding.ysclEdit.getText().toString());
            jsonObject.put("transportStartTime",binding.yskssjEdit.getText().toString()+" 00:00:00");
            jsonObject.put("origin",binding.ysqdEdit.getText().toString());
            jsonObject.put("terminus",binding.yszdEdit.getText().toString());
            jsonObject.put("expectTransportTime",binding.yshfEdit.getText().toString());
            jsonObject.put("remark",binding.bzEdit.getText().toString());
            jsonObject.put("id",BX_ID);
            if (imglist.size()>0){
        ...
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "/forestry/api/sickTreeTransportInfo");
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        Toast.makeText(AddbsysActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }else {
                        Toast.makeText(AddbsysActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
...
//        Log.e(TAG, "onActivityResult:data ",data);
            ...
            Bitmap photo = null;
            try {
                photo = ImageUtils.getBitmapFormUri(AddbsysActivity.this, uri);
            } catch (IOException e) {
            }
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        ...
                        Toast.makeText(AddbsysActivity.this, "提交失败", Toast.LENGTH_SHORT).show();
                        progressDialog.dismiss();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
