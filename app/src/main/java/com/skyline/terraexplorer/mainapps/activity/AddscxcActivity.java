...
import com.skyline.terraexplorer.databinding.ActivityAddscxcBinding;
...
public class AddscxcActivity extends BaseActivity implements DatePicker.OnDateChangedListener{
    private ActionBar actionBar;
    private ActivityAddscxcBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_addscxc);
        initView();
        initDateTime();
    }
    ...
    private void initView() {
                ...
                .subscribe(new Action1<Void>() {
                    ...
                    public void call(Void aVoid) {
                        if (uriChooseList.size() == 2){
                            Toast.makeText(AddscxcActivity.this, "最多可以添加两张图片", Toast.LENGTH_SHORT).show();
                        }else {
                            addImage();
                        }
                    }
        ...
        RxViewAction.clickNoDouble(binding.pclxEdit)
                .subscribe(new Action1<Void>() {
                    @Override
                    public void call(Void aVoid) {
                        showdclxChangeDailog();
                    }
                });
    ...
    private void postDataToService() {
        if (binding.slmjEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写松林面积", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        if (binding.dcmjEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写调查面积", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        if (binding.qyzsEdit.getText().toString().equals("")){
            Toast.makeText(this, "请填写取样株数", Toast.LENGTH_SHORT).show();
            progressDialog.dismiss();
            return;
        }
        if (binding.swslEdit.getText().toString().equals("")){
        ...
        try {
            ...
            jsonObject.put("isHadNematode",yqrw);
            jsonObject.put("surveyTime",binding.jianchashijianView.getText().toString()+" 00:00:00");
            jsonObject.put("address",binding.quxianEdit.getText().toString()+binding.xzjdEdit.getText().toString()+binding.cjwhEdit.getText().toString());
            jsonObject.put("smallClassNo",binding.yqxbEdit.getText().toString());
            jsonObject.put("pineArea",Double.parseDouble(binding.slmjEdit.getText().toString()));
            jsonObject.put("surveyArea",Double.parseDouble(binding.dcmjEdit.getText().toString()));
            jsonObject.put("samplingNumber",Integer.parseInt(binding.qyzsEdit.getText().toString()));
            jsonObject.put("deadPineNumber",Integer.parseInt(binding.swslEdit.getText().toString()));
        ...
        RequestParams params = new RequestParams(RequestUtils.REQUEST_URL + "/forestry/api/pestPineNematodeMonitor" );
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        Toast.makeText(AddscxcActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }else {
                        Toast.makeText(AddscxcActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
...
//        Log.e(TAG, "onActivityResult:data ",data);
            ...
            Bitmap photo = null;
            try {
                photo = ImageUtils.getBitmapFormUri(AddscxcActivity.this, uri);
            } catch (IOException e) {
            }
            ...
            public void onSuccess(String result) {
                ...
                try {
                    ...
                    if (code.equals("200")){
                        ...
                        Toast.makeText(AddscxcActivity.this, "提交失败", Toast.LENGTH_SHORT).show();
                        progressDialog.dismiss();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
    ...
    private void showdclxChangeDailog() {
        //默认选中第一个
        final String[] items = {"春季普查", "秋季普查"};
        ...
        builder = new AlertDialog.Builder(this).setTitle("选择区市")
                .setSingleChoiceItems(items, choose2, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        if (choose2 == 0) {
                            binding.pclxEdit.setText("春季普查");
                        } else if (choose2 == 1){
                            binding.pclxEdit.setText("秋季普查");
                        }
                    }
                });
        builder.create().show();
