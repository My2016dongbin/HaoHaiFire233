    ...
    public void onPlanCheckItemButtonClick(int state, Plan plan) {  ////state 0隐患排查 1资源检查  2导航
        if (state == 0){
           ...
           /* Log.e(TAG, "onPlanCheckItemButtonClick: lng=" + plan.getResourcePosition().getLng() );
            ...
            com.skyline.terraexplorer.utils.LatLng latLng = new LatLngChange().transformFromWGSToGCJ(new com.skyline.terraexplorer.utils.LatLng(plan.getLat(),plan.getLng()));
...
/*        List<Poi> poiList = new ArrayList();
        ...
        for (int i = 0; i < planList.size(); i++) {
            if (i == planList.size() - 1){
                //终点
                com.skyline.terraexplorer.utils.LatLng latLng = new LatLngChange().transformFromWGSToGCJ(new com.skyline.terraexplorer.utils.LatLng(planList.get(i).getLat(),planList.get(i).getLng()));
