...
public class WhiteboardListActivity extends MatchParentActivity implements ModalDialog.ModalDialogDelegate {
	...
	private void bindData() {
	    ...
	    for (int i=0; i<delegate.boards.size(); i++) {
	        WhiteboardTool.Whiteboard board = delegate.boards.get(i);
	...
	private void didSelectRowAtIndexPath(long packedPosition)
	{
	    ...
	    {
	    	WhiteboardTool.Whiteboard board = delegate.boards.get(item.tag);
	...
	public void modalDialogDidDismissWithOk(ModalDialog dlg) {
	    ...
	    switch (modalDialogMode) {
	        ...
	        case MD_ADD:
	        {
	            WhiteboardTool.Whiteboard board = new WhiteboardTool.Whiteboard(dlg.getTextField().getText().toString());
