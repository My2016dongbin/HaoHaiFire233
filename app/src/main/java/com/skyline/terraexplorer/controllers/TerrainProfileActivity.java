...
public class TerrainProfileActivity extends MatchParentActivity {
    ...
    private class TerrainProfileTileGenerator {
        ...
        private void drawVerticalLineForDataPoint(Canvas canvas, int index, String template, Paint paint, int scaledWidth, int scaledHeight, int col, int row) {
            ...
            // 现在画文字。 由于文本受尺度的影响，我们必须转换x，y并重置矩阵
            float[] point = new float[]{x, y};
            m.mapPoints(point);
            x = point[0];
            y = point[1];
            canvas.setMatrix(null);
            // a little pointer under text block 文本块下方的一个小指针
            RectF pointer = new RectF(x - 5, y - 10, x + 5, y);
            // draw pointer under text block 在文本块下绘制指针
            ...
            // add space for pointer under text block 在文本块下添加指针空间
            y = y - 5;
            ...
            // add spacing between lines 添加行间距
            lineHeight += 4;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        profileData = (ProfileTool.TerrainProfileData) getIntent().getSerializableExtra(ProfileTool.PROFILE_DATA);
    ...
    private void addOrReplaceChartView() {
        ...
        chartContainer.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                // tileView的顺序设置属性非常重要，它在addDetailLevel中做了很多工作，所以它必须是我们设置的最后一件事
                // 所有其他重要设置（setDecoder，setSize）都被设置
                ...
                // size of original image at 100% scale 原始图像尺寸为100%
