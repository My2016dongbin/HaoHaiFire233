...
//        UI.addHeader(R.string.title_activity_layers, R.drawable.layers, this, EnumSet.of(HeaderOptions.SearchButton));
        UI.addHeader(R.string.title_activity_layers, R.drawable.layers, this, options);
...
//        segmentedControl.setButtons(new int[]{R.string.layers_show_by_type});  // 停掉按组排序
        ...
        return UI.runOnRenderThread(new Callable<DisplayGroupItem[]>() {
            ...
            public DisplayGroupItem[] call() throws Exception {
                if (objects == null) {
                    ...
                    // create objects checkbox 创建对象复选框
                ...
                return new DisplayGroupItem[]{objects,featureLayers};
            }
    ...
    private void readLayers() {
        // walk over TE tree and find all location and presentation 走过树，找到所有的位置和演示文稿
...
//                            accessoryIcon = R.drawable.edit_layer;
                    ...
                    // for modify terrain objects, just save them in arary for later 为了修改地形对象，只需将其保存在数组中即可
    ...
    private void didSelectRowAtIndexPath(final long packedPosition) {
        ...
        UI.runOnRenderThread(new Runnable() {
            ...
            public void run() {
                ...
                // you can not turn on both slope map and contour map 您不能打开斜率图和轮廓图
