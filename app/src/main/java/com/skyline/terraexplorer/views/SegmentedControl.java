...
public class SegmentedControl extends LinearLayout {
    ...
    private void init() {
        // we must disable hardware acceleration for this view  我们必须禁用此视图的硬件加速
        // so we can use  canvas.clipPath(clipPath); in dispatchDraw 所以我们可以使用canvas.clipPath（clipPath）; 在dispatchDraw
    ...
    private void addButton(String text, int index, float weight) {
        if (index > 0) // add separator
        {
            ...
            addView(separator, new LayoutParams(1, LayoutParams.MATCH_PARENT));
        }
        ...
        ((LayoutParams) button.getLayoutParams()).weight = weight;
    }
