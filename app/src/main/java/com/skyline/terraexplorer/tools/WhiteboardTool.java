...
//        toolContainer.setText(String.format(TEApp.getAppContext().getString(R.string.whiteboard_tooltext), getCurrentBoard().name));
    ...
    private void loadBoardsDefinitions() {
        ...
        if (new File(boardsFile).exists()) {
            try {
                ...
                boards = (ArrayList<Whiteboard>) is.readObject();
                is.close();
            } catch (Exception ex) {
                ex.printStackTrace();
        ...
        if (boards == null) {
            boards = new ArrayList<Whiteboard>();
    ...
    public void saveBoardList() {
        ...
        try {
            ...
            ex.printStackTrace();
            Toast.makeText(TEApp.getAppContext(), "无法保存白板列表", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private IFeatureLayer loadFeatureLayer(String name) {
        ...
        // set altitude method - relative to terrain 设置高度法 - 相对于地形
    ...
    private void createFeatureLayerSHP(String name, int geomType) {
        ...
        // now lets see where TE created the layer 现在让我们看看TE创建的层面
        ...
        // and copy it to documents dir 并将其复制到文件目录中
    ...
    public void updateFeaturesVisibility() {
        ...
        UI.runOnRenderThread(new Runnable() {
            ...
            public void run() {
                ...
                } else // something visible 显示一些时事情
                {
    ...
    private void drawOnCurrentBoard(String layerId) {
        if (getCurrentBoard().visible == true) {
            ...
            // current board is hidden, ask user of he want to show it or create new board 当前板是隐藏的，要求用户想要显示它或创建新的板
    ...
    public void modalDialogDidDismissWithOk(ModalDialog dlg) {
        // ok means create new board 好的意思是创建新的主板
    ...
    public void modalDialogDidDismissWithCancel(ModalDialog dlg) {
        // cancel means make current board visible 取消手段使当前板可见
    ...
    public void deleteBoard(Whiteboard board) {
        // first remove all board features 首先删除所有的板功能
        ...
        // second remove the board itself 第二个删除板本身
        boards.remove(board);
        saveBoardList();
