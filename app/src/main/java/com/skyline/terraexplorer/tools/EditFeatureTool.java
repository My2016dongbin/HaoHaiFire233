...
import com.skyline.teapi.FeatureState;
...
import com.skyline.teapi.SGGeometryTypeId;
...
public class EditFeatureTool extends BaseToolWithContainer implements ModalDialogDelegate {
    ...
    public void open(final Object param) {
        UI.runOnRenderThread(new Runnable() {
            ...
            public void run() {
                ...
                // do not call slEDIT_ITEM_VERTICES on point. Causes problems in TE 不要在点中调用slEDIT_ITEM_VERTICES，导致TE中的问题
                ...
                // back up geometry for undo 备份几何以进行撤销
    ...
    public boolean onBeforeOpenToolContainer() {
        //Add feature overrides the method and calls super.onBeforeOpenToolContainer 添加功能覆盖该方法并调用super.onBeforeOpenToolContainer
        // and there feature may be null 并且功能可能为null
...
               所以让我们忽略这个请求 */
        ...
        // 如果功能已更改（或新功能），用户未确认保存，请询问用户并取消关闭
    ...
    private void saveChanges() {
        UI.runOnRenderThread(new Runnable() {
            ...
            public void run() {
                ...
                // if we are here, just need to save the layer 如果我们在这里，只需要保存图层
                try {
                    layer.Save();
                } catch (Exception e) {
