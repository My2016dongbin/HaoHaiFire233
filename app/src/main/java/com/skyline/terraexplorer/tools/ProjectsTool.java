...
public class ProjectsTool extends BaseTool {
	...
	public static void loadProject(final String path) {
		...
		// 打开项目可能会显示错误对话框。 如果发生这种情况太快（即找不到文件，不支持飞行）
		// 错误对话框将与调用活动（等）ProjectActivity相关联
		// 并且由于调用活动将很快完成，它会导致“android activity has leaked window”
		// 即尝试打开不支持的文件，错误，显示项目活动，再次打开不支持的文件，
		// 错误对话框与正在关闭并导致泄漏异常的项目活动相关联。
		// 修复它，在打开新项目之前总是添加延迟
