...
public class ShadowTool extends BaseToolWithContainer {
    ...
    // slider goes from 6 am to 10 pm 滑块从上午6点到晚上10点
        ...
        public boolean onTouch(View v, MotionEvent event) {
            ...
            switch (action) {
                case MotionEvent.ACTION_DOWN:
                    // Disallow ScrollView to intercept touch events.  禁止ScrollView拦截触摸事件。
                    toolContainerHorizontalScrollView.requestDisallowInterceptTouchEvent(true);
                    break;

                case MotionEvent.ACTION_UP:
                    // Allow ScrollView to intercept touch events. 允许ScrollView拦截触摸事件。
                    toolContainerHorizontalScrollView.requestDisallowInterceptTouchEvent(false);
                    break;
            }

            // Handle slider touch events. 处理滑块触摸事件。
    ...
    private void sliderMoved(int progress) {
        // disable shadow while moving slider 移动滑块时禁用阴影
        UI.runOnRenderThreadAsync(new Runnable() {
            @Override
            public void run() {
        ...
        // slider value is 16 hours, i.e 16*60 seconds 滑块值为16小时，即16*60秒
    ...
    private void sliderMoveEnded() {
        // enable shadow after moved finished 移动完成后启用阴影
        UI.runOnRenderThreadAsync(new Runnable() {
            @Override
            public void run() {
