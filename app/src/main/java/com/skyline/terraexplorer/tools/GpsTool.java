...
import com.skyline.teapi.GPSOperationMode;
...
public class GpsTool extends BaseTool implements LocationTrackerDelegate, OnLoadFinishedListener, OnSGWorldListener {
    ...
    public GpsTool() {
        ...
        locationLockButton.setBackgroundResource(R.drawable.selector_rl_back);
//        locationLockButton.setBackgroundColor(locationLockButton.getResources().getColor(R.color.color_control_background));
    ...
    private void setCameraLocked(final boolean cameraLocked) {
        UI.runOnUiThreadAsync(new Runnable() {        //在主线程
            @Override
            public void run() {
    ...
    private void startGPSIconBlinking() {
        ...
        h.postDelayed(new Runnable() {
            @Override
            public void run() {
                // if after two seconds camera still locked and we do not have a connection　如果两秒钟后相机仍然锁定并且我们没有连接
    ...
    private void updateGpsTrail(Location location) {
        ...
        // add new segment to the trail once per 4 seconds　每4秒向轨迹中添加新的段
        ...
        } else // otherwise just move last point in last segment to current location　否则只需将最后一个段的最后一个位置移动到当前位置
        {
            ILineString geom = ISGWorld.getInstance().getCreator().getGeometryCreator().CreateLineStringGeometry(new double[]{
        ...
        // limit gps trail to 100 segments　将gps跟踪限制在100个细分
        ...
        // add point to gps points layer　加点到gps点图层
        if (gpsTrailFeatureLayer == null) {
            // if gps points file does not exists, create it and copy to documents　如果gps点文件不存在，创建它并复制到文档
            ...
            if ((new File(gpsPointsFile)).exists() == false) {
                ...
                layer.Save();
                // remove the layer from TE　从TE中删除图层
            ...
            ISGWorld.getInstance().getProjectTree().SetVisibility(gpsTrailFeatureLayer.getID(), false);
                ...
                Log.e("updateGpsTrail",e.getMessage());
            }
        ...
        // save gps points layer once per 30 seconds　每30秒保存一次gps点图层
