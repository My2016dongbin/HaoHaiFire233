...
public class LazyViewPager extends ViewGroup {
    ...
    private static final Comparator<ItemInfo> COMPARATOR = new Comparator() {
        ...
        public int compare(LazyViewPager.ItemInfo lhs, LazyViewPager.ItemInfo rhs) {
    ...
    private LazyViewPager.PagerObserver mObserver;
    ...
    public void setAdapter(PagerAdapter adapter) {
        if(this.mAdapter != null) {
            ...
            for(int i = 0; i < this.mItems.size(); ++i) {
                LazyViewPager.ItemInfo ii = (LazyViewPager.ItemInfo)this.mItems.get(i);
        ...
        if(this.mAdapter != null) {
            if(this.mObserver == null) {
                this.mObserver = new LazyViewPager.PagerObserver();
            }
            ...
            if(!always && this.mCurItem == item && this.mItems.size() != 0) {
                ...
                if(item > this.mCurItem + pageLimit || item < this.mCurItem - pageLimit) {
                    for(int dispatchSelected = 0; dispatchSelected < this.mItems.size(); ++dispatchSelected) {
                        ((LazyViewPager.ItemInfo)this.mItems.get(dispatchSelected)).scrolling = true;
                    }
                }
    ...
    void addNewItem(int position, int index) {
        LazyViewPager.ItemInfo ii = new LazyViewPager.ItemInfo();
            ...
            if(!this.mPopulatePending) {
                if(this.getWindowToken() != null) {
                    ...
                    for(int curItem = 0; curItem < this.mItems.size(); ++curItem) {
                        LazyViewPager.ItemInfo currentFocused = (LazyViewPager.ItemInfo)this.mItems.get(curItem);
                    ...
                    lastPos = this.mItems.size() > 0?((LazyViewPager.ItemInfo)this.mItems.get(this.mItems.size() - 1)).position:-1;
                    if(lastPos < endPos) {
                        ++lastPos;
                    ...
                    LazyViewPager.ItemInfo var12 = null;

                    for(int var11 = 0; var11 < this.mItems.size(); ++var11) {
                        if(((LazyViewPager.ItemInfo)this.mItems.get(var11)).position == this.mCurItem) {
                            var12 = (LazyViewPager.ItemInfo)this.mItems.get(var11);
                            break;
                        }
                    }
                    ...
                    if(this.hasFocus()) {
                        ...
                        LazyViewPager.ItemInfo ii = var13 != null?this.infoForAnyChild(var13):null;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        LazyViewPager.SavedState ss = new LazyViewPager.SavedState(superState);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if(!(state instanceof LazyViewPager.SavedState)) {
            super.onRestoreInstanceState(state);
        } else {
            LazyViewPager.SavedState ss = (LazyViewPager.SavedState)state;
    ...
    LazyViewPager.ItemInfo infoForChild(View child) {
        for(int i = 0; i < this.mItems.size(); ++i) {
            LazyViewPager.ItemInfo ii = (LazyViewPager.ItemInfo)this.mItems.get(i);
    ...
    LazyViewPager.ItemInfo infoForAnyChild(View child) {
        while(true) {
            ViewParent parent;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        this.mChildWidthMeasureSpec = MeasureSpec.makeMeasureSpec(this.getMeasuredWidth() - this.getPaddingLeft() - this.getPaddingRight(),  MeasureSpec.EXACTLY);
        this.mChildHeightMeasureSpec = MeasureSpec.makeMeasureSpec(this.getMeasuredHeight() - this.getPaddingTop() - this.getPaddingBottom(), MeasureSpec.EXACTLY);
        this.mInLayout = true;
        this.populate();
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for(int i = 0; i < count; ++i) {
            View child = this.getChildAt(i);
            LazyViewPager.ItemInfo ii;
            if(child.getVisibility() != GONE && (ii = this.infoForChild(child)) != null) {
    ...
    private void completeScroll() {
        ...
        for(i = 0; i < this.mItems.size(); ++i) {
            LazyViewPager.ItemInfo var6 = (LazyViewPager.ItemInfo)this.mItems.get(i);
            if(var6.scrolling) {
                needPopulate = true;
        ...
        if(descendantFocusability != 393216) {
            for(int i = 0; i < this.getChildCount(); ++i) {
                ...
                if(child.getVisibility() == VISIBLE) {
                    LazyViewPager.ItemInfo ii = this.infoForChild(child);
                    if(ii != null && ii.position == this.mCurItem) {
    ...
    public static class SavedState extends BaseSavedState {
        ...
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new ParcelableCompatCreatorCallbacks() {
            public LazyViewPager.SavedState createFromParcel(Parcel in, ClassLoader loader) {
                return new LazyViewPager.SavedState(in, loader);
            }

            public LazyViewPager.SavedState[] newArray(int size) {
                return new LazyViewPager.SavedState[size];
            }
        });
    ...
    public static class SimpleOnPageChangeListener implements LazyViewPager.OnPageChangeListener {
