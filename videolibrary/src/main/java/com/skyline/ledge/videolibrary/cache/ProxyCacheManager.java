...
import com.skyline.ledge.videolibrary.utils.StorageUtils;
import com.skyline.ledge.videolibrary.videocatch.HttpProxyCacheServer;
...
public class ProxyCacheManager implements ICacheManager, CacheListener {
    ...
    public void doCacheLogic(Context context, IMediaPlayer mediaPlayer, String originUrl, Map<String, String> header, File cachePath) {
        ...
        if (url.startsWith("http") && !url.contains("127.0.0.1") && !url.contains(".m3u8")) {
            ...
            if (proxy != null) {
                ...
                //注册上缓冲监听
                if (!mCacheFile) {
                    proxy.registerCacheListener((com.skyline.ledge.videolibrary.videocatch.CacheListener) this, originUrl);
                }
            }
    ...
    public void release() {
        if (proxy != null) {
            try {
                proxy.unregisterCacheListener((com.skyline.ledge.videolibrary.videocatch.CacheListener) this);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
